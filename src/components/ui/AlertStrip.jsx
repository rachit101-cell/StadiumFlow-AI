...
/**
 * AlertStrip — horizontal strip of live venue alerts.
 * Uses aria-live="polite" for the general log and aria-live="assertive"
 * for a hidden screen-reader-only zone that announces critical alerts immediately.
 */
export const AlertStrip = () => {
  const { venueState, dispatch } = useVenue();

  const sorted = useMemo(() => (
    [...venueState.activeAlerts].sort((a, b) => {
      const so = (severityOrder[a.severity] ?? 4) - (severityOrder[b.severity] ?? 4);
      if (so !== 0) return so;
      return a.timestamp - b.timestamp;
    })
  ), [venueState.activeAlerts]);

  const latestCritical = useMemo(() => (
    sorted.find(a => a.severity === 'critical') || null
  ), [sorted]);

  return (
    <>
      {/* Screen-reader assertive region — announces critical alerts immediately */}
      <div
        role="alert"
        aria-live="assertive"
        aria-atomic="true"
        className="sr-only"
        id="critical-alert-announcer"
      >
        {latestCritical ? `Critical alert: ${latestCritical.message}` : ''}
      </div>

      {/* Main visible alert strip — polite live region */}
      <div
        role="log"
        aria-live="polite"
            ...
            sorted.map(alert => {
              ...
              return (
                ...
                >
                  ...
                  <span className="sr-only">{`${alert.severity} severity alert: ${alert.message}`}</span>
                </motion.div>
              );
            })
          )}
